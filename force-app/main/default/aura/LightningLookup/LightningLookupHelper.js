({
	...
	hlpSelectItem : function(component, index){
        try{
            ...
            if(matchedListDisplay[index].toLowerCase() != 'no records found!'){
                ...
                this.fireUpdate(component.get('v.cmpId'), matchedListRecords[index],matchedListValue[index],matchedListDisplay[index], component);
            }
     ...
     */
    fireUpdate : function(name, record, recordId, recordName, component){
        console.log('EVENT: EvtChangeLookup');
        var ev = component.getEvent("updateLookup");//$A.get('e.c:EvtChangeLookup');
        ev.setParams({
            'name' : name,
            'record' : record,
            'recordId' : recordId,
     ...
     * @param  {[String]} name  [component id]
     */
    fireClear : function(name, component){
        console.log('EVENT: EvtClearLookup');
        var ev = component.getEvent("clearLookup");//$A.get('e.c:EvtClearLookup');
        ev.setParams({
            'name' : name
        });
        ev.fire();
     ...
     * @param  {[String]} name  [component id]
     */
    fireInit : function(name, component){
        console.log('EVENT: EvtInitLookup');
        var ev = component.getEvent("initLookup");//$A.get('e.c:EvtInitLookup');
        ev.setParams({
            'name' : name
        });
        ev.fire();
    ...
    populateField : function(component,name,val){
        try{
            console.log(':::populateField--name='+ name + '-val='+ val);
            ...
            this.fireInit(component.get('v.cmpId'), component);
        }
        catch(e){
            this.showError(component, e.message);
        }
    ...
    initField: function(component){
        ...
        console.log(':::initField--label='+ label + '--val='+ val);
        if(label != null && (val ==null || val =='' ||$A.util.isEmpty(val) || $A.util.isUndefined(val))){
           	this.clearField(component,true);
			this.toggleIcons(component,true);
            label = '';
        }
       
        if(label && val){
            console.log(':::initField--If:::');
            this.populateField(component,label,val);
        }
        else{
            console.log(':::initField--Else:::');
            action.setParams({
            ...
            action.setCallback(this,function(response){
                ...
                //By Default First Result will be displayed
                if(
                    res.lstDisplay != null 
                    //&& label !=null && !$A.util.isEmpty(label) && !$A.util.isUndefined(label)
                    &&  val !=null && val !='' && !$A.util.isEmpty(val) && !$A.util.isUndefined(val)
                  )
	...
	showDropDown : function(component,toggle){
        try{
            ...
            this.toggleIcons(component,true);
            console.log(':::diplayedul:::');
            //if(!$A.util.isEmpty(component.find('diplayedul').get('v.body')) && !$A.util.isUndefined(component.find('diplayedul').get('v.body')) )
    ...
    clearField : function(component, fireEvent){
        try{
            ...
            if(fireEvent)
                this.fireClear(component.get('v.cmpId'), component);
        }
        catch(e){
            this.showError(component, e.message);
        }
